...
use super::row::SnowflakeColumnType;
use super::{Error, Result, SnowflakeRow, chunk::download_chunk};
...
pub struct QueryExecutor {
    ...
    /// Snowflake query id, captured at submit so a long-running query can be
    /// reattached via RESULT_SCAN even if the wait is interrupted.
...
impl QueryExecutor {
    pub(super) async fn create<Q: Into<QueryRequest>>(
        ...
        let mut response: SnowflakeResponse = {
            ...
            loop {
                ...
                if !status.is_success() && parsed.code.as_deref() != Some(SESSION_EXPIRED) {
        ...
        // Capture the query id from the submit response (it's present before
        // the poll), so an interrupted wait can still point at RESULT_SCAN.
            ...
            || response_code == Some(QUERY_IN_PROGRESS_CODE)
        {
            ...
            match data.get_result_url {
                Some(result_url) => {
                    response = match poll_for_async_results(sess, &result_url, base_url.clone())
                        .await
                    {
                        Ok(r) => r,
                        // The query is still running server-side; surface its id
                        // so the user can reattach with RESULT_SCAN.
                        Err(e) => return Err(interrupted(query_id.clone(), e)),
                    }
                }
                None => {
    ...
    /// The Snowflake query id of this statement, if known.
...
/// Wrap a poll/transport error as `QueryInterrupted` (carrying the reattach
/// hint) when the query id is known; otherwise pass the error through.
fn interrupted(query_id: Option<String>, e: Error) -> Error {
    match query_id {
        Some(qid) => Error::QueryInterrupted {
            query_id: qid,
            detail: e.to_string(),
        },
        None => e,
